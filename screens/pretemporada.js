...
const bancoCartas = [
    { titulo:"CURSO RÁPIDO DE PREMIERE", tipo:"RARA", attr:"edicion", pts:4, desc:"Aprendés a cortar silencios, mejorar ritmo y hacer que cada segundo cuente.", color:"var(--accent-yellow)" },
    ...
    { titulo:"CURSO DE TEATRO E IMPRO", tipo:"RARA", attr:"carisma", pts:4, desc:"Aprendés a soltarte y conectar mejor con la cámara.", color:"var(--accent-yellow)" },
    { titulo:"ESTRATEGIA DE CONTENIDO", tipo:"COMÚN", attr:"marketing", pts:3, desc:"Aprendés a pensar títulos, clips y distribución para atraer audiencia.", color:"var(--accent-green)" },
    { titulo:"DISCIPLINA DE STREAMER", tipo:"ÉPICA", attr:"constancia", pts:5, desc:"Horarios fijos y rutina estricta para sostener tu canal durante el año.", color:"var(--accent-red)" },
    { titulo:"CURSO DE IDEAS VIRALES", tipo:"ÉPICA", attr:"creatividad", pts:5, desc:"Entrenás el músculo de encontrar conceptos que hagan parar el scroll.", color:"var(--accent-red)" },
    { titulo:"NETWORKING EN EVENTOS", tipo:"COMÚN", attr:"networking", pts:3, desc:"Aprendés a hacer contactos y abrir puertas con otros creadores.", color:"var(--accent-green)" }
];
...
export function renderPretemporada(el) {
    ...
    container.innerHTML = `
        <div class="page-shell">
            ...
            <div class="cards-grid">
                ${opciones.map((carta,index)=>`<div class="preseason-card panel" style="border-top:3px solid ${carta.color};margin:0;min-height:330px;display:flex;flex-direction:column;justify-content:space-between;"><div><span class="card-type" style="background:${carta.color};color:#000;padding:4px 8px;border-radius:5px;font-size:.68rem;font-weight:900;display:inline-block;">${carta.tipo}</span><h2>${carta.titulo}</h2><p class="muted">${carta.desc}</p></div><div><div class="card-bonus" style="text-align:center;color:var(--accent-green);font-weight:900;margin:18px 0;">+${carta.pts} ${nombres[carta.attr]}</div><button class="btn primary select-card-button" data-index="${index}" style="width:100%;">ELEGIR MEJORA</button></div></div>`).join("")}
            </div>
        </div>`;
    ...
    container.querySelectorAll(".select-card-button").forEach(button => button.addEventListener("click", () => {
        ...
        gameState.player.pretemporada = { año, entrenamiento:carta.titulo, atributo:carta.attr, puntos:carta.pts };
